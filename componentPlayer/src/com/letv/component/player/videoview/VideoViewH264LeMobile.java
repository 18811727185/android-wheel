...
public class VideoViewH264LeMobile extends SurfaceView implements LetvMediaPlayerControl {
	private String TAG = "VideoViewLeMobile";
...
//				height = (int) Math.ceil(height / ratios);
	...
	public void setVideoURI(Uri uri) {
		...
		LetvMediaPlayerManager.getInstance().writePlayLog("系统当前时间:  "+currentDate+" VideoViewH264LeMobile(乐视手机videoview)  setVideoURI(), url="
...
//			PreferenceUtil.setErrorCode(mContext, "VideoViewH264LeMobile error, framework_err=" + framework_err + ", impl_err=" + impl_err);
			String currentDate = Tools.getCurrentDate();
...
//				setVisibility(View.VISIBLE);
				...
				LetvMediaPlayerManager.getInstance().writePlayLog("系统当前时间:  "+currentDateStop+"VideoViewH264LeMobile start()");
	...
	public void pause() {
		if (isInPlaybackState()) {
			if (mMediaPlayer.isPlaying()) {
				...
				mMediaPlayer.pause();
				String currentDateStop = Tools.getCurrentDate();
				LetvMediaPlayerManager.getInstance().writePlayLog("系统当前时间:  "+currentDateStop+"VideoViewH264LeMobile pause()");
