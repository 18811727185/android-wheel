...
public class VideoViewH264m3u8Hw extends SurfaceView implements LetvMediaPlayerControl {
	...
	public void setVideoURI(Uri uri) {
		...
		LetvMediaPlayerManager.getInstance().writePlayLog("系统当前时间:  "+currentDate+" VideoViewH264m3u8Hw(硬解m3u8)  setVideoURI(), url="
...
//				setVisibility(View.VISIBLE);
				...
				mMediaPlayer.start();
				String currentDate = Tools.getCurrentDate();
				LetvMediaPlayerManager.getInstance().writePlayLog("系统当前时间:  "+currentDate+" VideoViewH264m3u8Hw(硬解m3u8)  start()");
			}
		} else {
	...
	public void pause()
	{
		if (isInPlaybackState())
		{
			if (mMediaPlayer.isPlaying())
			{
				...
				mMediaPlayer.pause();
				String currentDate = Tools.getCurrentDate();
				LetvMediaPlayerManager.getInstance().writePlayLog("系统当前时间:  "+currentDate+" VideoViewH264m3u8Hw(硬解m3u8)  pause()");
...
//			PreferenceUtil.setErrorCode(mContext, "VideoViewH264m3u8Hw error, framework_err=" + framework_err + ", impl_err=" + impl_err);
	...
	private FFMpegPlayer.OnBlockListener mBlockListener = new FFMpegPlayer.OnBlockListener() {
		...
		public void onBlock(FFMpegPlayer mediaPlayer, int blockinfo) {
			...
			if(blockinfo == FFMpegPlayer.MEDIA_BLOCK_START) {
				String currentDate = Tools.getCurrentDate();
				LetvMediaPlayerManager.getInstance().writePlayLog("系统当前时间:  "+currentDate+" VideoViewH264m3u8Hw(硬解m3u8)  出现卡顿");
			} else if(blockinfo == FFMpegPlayer.MEDIA_BLOCK_END) {
				String currentDate = Tools.getCurrentDate();
				LetvMediaPlayerManager.getInstance().writePlayLog("系统当前时间:  "+currentDate+" VideoViewH264m3u8Hw(硬解m3u8)  结束卡顿");
			}
		}
	};
