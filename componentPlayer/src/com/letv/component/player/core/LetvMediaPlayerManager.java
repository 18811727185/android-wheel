...
public class LetvMediaPlayerManager {
    ...
    /**
     * 支持码流级别，如一款手机，将返回支持最大码流级别SUPPORT_TS720P_LEVEL，那么SUPPORT_TS720P_LEVEL之下的都将支持，
     * SUPPORT_TS720P_LEVEL之上的码流不支持
     */
    ...
    /**
     * 获取硬解支持能力类型
     * @return HARD_DECODE_BLACK, HARD_DECODE_WHITE, HARD_DECODE_GRAY_CONFIGURABLE,
     *         HARD_DECODE_GRAY_UNCONFIGURABLE
     */
    ...
    public int getHardDecodeSupportLevel() {
         ...
         * } else
         */if (mConfig.mHardDecodeCapability.isSupport720p) {
     ...
     * 返回软解支持的最高码流
     * @return
     *         SUPPORT_TS1300K_LEVEL，SUPPORT_TS1000K_LEVEL，SUPPORT_TS350K_LEVEL，SUPPORT_TS180K_LEVEL
     *         ,
     *         SUPPORT_MP4_LEVEL
     */
    ...
    public void feedbackCommit() {
            ...
            public void response(String res) {
                ...
                try {
                    ...
                    if (object != null) {
                        ...
                        boolean supportHWDecodeUseNative = HardDecodeUtils
                                .isSupportHWDecodeUseNative();
                        int avcLevel = HardDecodeUtils.getAVCLevel();
                        PreferenceUtil.setLocalCapcity(mContext,
                                "本地解码能力：supportHWDecodeUseNative  " + supportHWDecodeUseNative
                                        + "  avcLevel = " + avcLevel);
                        if (mExecutorService == null) {
                            mExecutorService = Executors
                                    .newCachedThreadPool(new NamedDefaultThreadFactory(
                                            LetvMediaPlayerManager.class.getName()));
                        }
        ...
        }).executeOnExecutor(Executors.newCachedThreadPool(new NamedDefaultThreadFactory(
                LetvMediaPlayerManager.class.getName())), "");
    }
    ...
    private synchronized static void upLog(String path, String fbid) {
        ...
        if (!fileParent.exists()) {
            ...
            if (files.size() == 0) {
                ...
                }, fbid).executeOnExecutor(Executors
                        .newCachedThreadPool(new NamedDefaultThreadFactory(
                                LetvMediaPlayerManager.class.getName())), "");
            }
        }
    ...
    private String getSaveDirc() {
        String pathDownload = mContext.getDir("LetvLog",
                Context.MODE_WORLD_WRITEABLE | Context.MODE_WORLD_READABLE | Context.MODE_PRIVATE)
                .getPath();
    ...
    public void writePlayLog(final String content, final boolean isRemoveHistoryFileInfo) {
        if (mExecutorService == null) {
            mExecutorService = Executors.newCachedThreadPool(new NamedDefaultThreadFactory(
                    LetvMediaPlayerManager.class.getName()));
        }
        mExecutorService.execute(new Runnable() {
            @Override
                ...
                try {
                    synchronized (LetvMediaPlayerManager.this) {
                        ...
                        FileWriter writer = new FileWriter(new File(
                                createFileIfNotExist(getSaveDirc())), true);
