...
public class DragSortHelper {
    ...
    private int mDropAnimDuration = 150;// ms
    ...
    public DragSortHelper(Context context, TypedArray typedArray, LeListView leListView) {
        init(context, typedArray, leListView);
    }
    
    private void init(Context context, TypedArray typedArray, LeListView leListView) {
        ...
        if (typedArray != null) {
            mDragEnabled = typedArray.getBoolean(R.styleable.LeListView_leSortDragEnabled, mDragEnabled);
            // alpha between 0 and 255, 0=transparent, 255=opaque
            mFloatAlpha = typedArray.getFloat(R.styleable.LeListView_leSortFloatAlpha, mFloatAlpha);
            mCurrFloatAlpha = mFloatAlpha;
            mSlideRegionFrac = Math.max(0.0f, Math.min(1.0f, 1.0f - typedArray.getFloat(R.styleable.LeListView_leSortSlideShuffleSpeed, 0.75f)));
            mAnimate = mSlideRegionFrac > 0.0f;
            float frac = typedArray.getFloat(R.styleable.LeListView_leSortDragScrollStart, mDragUpScrollStartFrac);
            setDragScrollStart(frac);
            mMaxScrollSpeed = typedArray.getFloat(R.styleable.LeListView_leSortMaxDragScrollSpeed, mMaxScrollSpeed);
            mDropAnimDuration = typedArray.getInt(R.styleable.LeListView_leSortDropAnimationDuration, mDropAnimDuration);
            
            boolean useDefaultController = typedArray.getBoolean(R.styleable.LeListView_leSortUseDefaultController, true);
            if (useDefaultController) {
                sortEnabled = typedArray.getBoolean(R.styleable.LeListView_leSortEnabled, true);
                dragInitMode = typedArray.getInt(R.styleable.LeListView_leSortDragStartMode, DragSortController.ON_DOWN);
                dragHandleId = typedArray.getResourceId(R.styleable.LeListView_leSortDragHandleId, 0);
                bgColor = typedArray.getColor(R.styleable.LeListView_leSortFloatBackgroundColor, Color.BLACK);
            }
        }
        
        mLeListView = leListView;
        
        DragSortController controller = new DragSortController(leListView, dragHandleId, dragInitMode);
        controller.setSortEnabled(sortEnabled);
        controller.setBackgroundColor(bgColor);
        mFloatViewManager = (FloatViewManager) controller;
        mLeListView.setOnTouchListener(controller);
        
        mDragScroller = new DragScroller();
        float smoothness = 0.5f;
        if (mDropAnimDuration > 0) {
            mDropAnimator = new DropAnimator(smoothness, mDropAnimDuration);
        }
    }
    ...
    private void drawDivider(int expPosition, Canvas canvas) {
        ...
        final Drawable divider = mLeListView.getDivider();
        final int dividerHeight = mLeListView.getDividerHeight();
        // Log.d(TAG_DRAG_SORT, "div="+divider+" divH="+dividerHeight);

        if (divider != null && dividerHeight != 0) {
            final ViewGroup expItem = (ViewGroup) mLeListView.getChildAt(expPosition - mLeListView.getFirstVisiblePosition());
            if (expItem != null) {
                final int l = mLeListView.getPaddingLeft();
                final int r = mLeListView.getWidth() - mLeListView.getPaddingRight();
                final int t;
                final int b;
                ...
                // Log.d(TAG_DRAG_SORT, "l="+l+" t="+t+" r="+r+" b="+b);
    ...
    void dispatchDraw(Canvas canvas) {
        ...
        if (mFloatView != null) {
            ...
            int width = mLeListView.getWidth();
            if (x < 0)
                x = -x;
            float alphaMod;
            if (x < width) {
            ...
            canvas.save();
            // Log.d(TAG_DRAG_SORT, "clip rect bounds: " + canvas.getClipBounds());
    ...
    private int getItemHeight(int position) {
        View v = mLeListView.getChildAt(position - mLeListView.getFirstVisiblePosition());
    ...
    /*private void printPosData() {
        Log.d(TAG_DRAG_SORT, "mSrcPos=" + mSrcPos + " mFirstExpPos=" + mFirstExpPos + " mSecondExpPos=" + mSecondExpPos);
    }*/
    ...
    private int getShuffleEdge(int position, int top) {
        ...
        final int numHeaders = mLeListView.getHeaderViewsCount();
        final int numFooters = mLeListView.getFooterViewsCount();

        // shuffle edges are defined between items that can be
        // dragged; there are N-1 of them if there are N draggable
        // items.

        if (position <= numHeaders || (position >= mLeListView.getCount() - numFooters)) {
            return top;
        }

        int divHeight = mLeListView.getDividerHeight();
    ...
    private boolean updatePositions() {
        ...
        final int first = mLeListView.getFirstVisiblePosition();
        int startPos = mFirstExpPos;
        View startView = mLeListView.getChildAt(startPos - first);

        if (startView == null) {
            startPos = first + mLeListView.getChildCount() / 2;
            startView = mLeListView.getChildAt(startPos - first);
        }
        ...
        if (mFloatViewMid < edge) {
            ...
            // Log.d(TAG_DRAG_SORT, "edge = " + edge);
            final int count = mLeListView.getCount();
            while (itemPos < count) {
                if (itemPos == count - 1) {
                    edge = itemTop + divHeight + itemHeight;
                    break;
        ...
        if (mAnimate) {
            ...
            // Log.d(TAG_DRAG_SORT, "edgeTop = " + edgeTop + " edgeBot = " + edgeBottom);
            ...
            if (mFloatViewMid < slideEdgeTop) {
                ...
                // Log.d(TAG_DRAG_SORT," firstExp = " + mFirstExpPos + " secExp = " + mSecondExpPos + " slideFrac = " + mSlideFrac);
        ...
        if (mFirstExpPos < numHeaders) {
            ...
            mFirstExpPos = itemPos;
            mSecondExpPos = itemPos;
        } else if (mSecondExpPos >= mLeListView.getCount() - numFooters) {
            itemPos = mLeListView.getCount() - numFooters - 1;
            mFirstExpPos = itemPos;
            mSecondExpPos = itemPos;
        }
    ...
    private class DropAnimator extends SmoothAnimator {
        ...
        public void onStart() {
            ...
            mInitDeltaX = mFloatLoc.x - mLeListView.getPaddingLeft();
        }

        private int getTargetY() {
            final int first = mLeListView.getFirstVisiblePosition();
            final int otherAdjust = (mItemHeightCollapsed + mLeListView.getDividerHeight()) / 2;
            View v = mLeListView.getChildAt(mDropPos - first);
            int targetY = -1;
            if (v != null) {
                if (mDropPos == srcPos) {
                    targetY = v.getTop();
        ...
        public void onUpdate(float frac, float smoothFrac) {
            ...
            final int targetX = mLeListView.getPaddingLeft();
            ...
            if (f < Math.abs(deltaY / mInitDeltaY) || f < Math.abs(deltaX / mInitDeltaX)) {
                ...
                mFloatLoc.x = mLeListView.getPaddingLeft() + (int) (mInitDeltaX * f);
                doDragFloatView(true);
            }
        }
     ...
     * through to {@link com.letv.shared.widget.DragSortHelper.DropListener#drop(int, int)}.
     * 
    ...
    public void moveItem(int from, int to) {
        if (mDropListener != null) {
            final int count = mLeListView.getInputAdapter().getCount();
    ...
    private void dropFloatView() {
        ...
        if (mDropListener != null && mFloatPos >= 0 && mFloatPos < mLeListView.getCount()) {
            final int numHeaders = mLeListView.getHeaderViewsCount();
    ...
    private void adjustOnReorder() {
        final int firstPos = mLeListView.getFirstVisiblePosition();
        // Log.d(TAG_DRAG_SORT, "first = " + firstPos + " src = " + mSrcPos);
        if (mSrcPos < firstPos) {
            // collapsed src item is off screen;
            // adjust the scroll after item heights have been fixed
            View v = mLeListView.getChildAt(0);
            int top = 0;
            if (v != null) {
                top = v.getTop();
            }
            // Log.d(TAG_DRAG_SORT, "top = " + top + " fvh = " + mFloatViewHeight);
            mLeListView.setSelectionFromTop(firstPos - 1, top - mLeListView.getPaddingTop());
        }
    }
    ...
    public void updateScrollStarts() {
        final int padTop = mLeListView.getPaddingTop();
        final int listHeight = mLeListView.getHeight() - padTop - mLeListView.getPaddingBottom();
    ...
    private void adjustAllItems() {
        final int first = mLeListView.getFirstVisiblePosition();
        final int last = mLeListView.getLastVisiblePosition();

        int begin = Math.max(0, mLeListView.getHeaderViewsCount() - first);
        int end = Math.min(last - first, mLeListView.getCount() - 1 - mLeListView.getFooterViewsCount() - first);

        for (int i = begin; i <= end; ++i) {
            View v = mLeListView.getChildAt(i);
            if (v != null) {
    ...
    private int getChildHeight(int position) {
        ...
        if (v != null) {
            ...
            if (childHeight != -1) {
                // Log.d(TAG_DRAG_SORT, "found child height in cache!");
                return childHeight;
            }

            final ListAdapter adapter = mLeListView.getAdapter();
            ...
            if (type >= 0) {
                if (mSampleViewTypes[type] == null) {
                    v = adapter.getView(position, null, mLeListView);
                    mSampleViewTypes[type] = v;
                } else {
                    v = adapter.getView(position, mSampleViewTypes[type], mLeListView);
                }
            } else {
    ...
    private int getChildHeight(int position, View item, boolean invalidChildHeight) {
        ...
        View child;
        if (position < mLeListView.getHeaderViewsCount() || position >= mLeListView.getCount() - mLeListView.getFooterViewsCount()) {
            child = item;
        } else {
    ...
    private void measureItem(View item) {
        ...
        int wspec = ViewGroup.getChildMeasureSpec(mWidthMeasureSpec, mLeListView.getListPaddingLeft()
                + mLeListView.getListPaddingRight(), lp.width);
        int hspec;
        if (lp.height > 0) {
    ...
    void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        // Log.d(TAG_DRAG_SORT, "onMeasure called");
        if (mFloatView != null) {
    ...
    protected boolean onDragTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                // Log.d(TAG_DRAG_SORT, "calling stopDrag from onDragTouchEvent");
                if (mDragState == DRAGGING) {
                    stopDrag();
                }
                doActionUpOrCancel();
    ...
    public boolean startDrag(int position, View floatView, int dragFlags, int deltaX, int deltaY) {
        ...
        if (mLeListView.getParent() != null) {
            mLeListView.getParent().requestDisallowInterceptTouchEvent(true);
        }

        int pos = position + mLeListView.getHeaderViewsCount();
        mFirstExpPos = pos;
        mSecondExpPos = pos;
        mSrcPos = pos;
        mFloatPos = pos;
        ...
        final View srcItem = mLeListView.getChildAt(mSrcPos - mLeListView.getFirstVisiblePosition());
        ...
        switch (mCancelMethod) {
            case ON_TOUCH_EVENT:
                ...
                mLeListView.onTouchEventSuper(mCancelEvent);
                mCancelEvent.recycle();
                break;
            case ON_INTERCEPT_TOUCH_EVENT:
                mCancelEvent = MotionEvent.obtain(0, 0, MotionEvent.ACTION_CANCEL, 0f, 0f, 0f, 0f, 0, 0f, 0f, 0, 0);
                mLeListView.onInterceptTouchEventSuper(mCancelEvent);
                mCancelEvent.recycle();
                break;
        }

        mLeListView.requestLayout();
    ...
    private void doDragFloatView(boolean forceInvalidate) {
        int movePos = mLeListView.getFirstVisiblePosition() + mLeListView.getChildCount() / 2;
        View moveItem = mLeListView.getChildAt(mLeListView.getChildCount() / 2);
    ...
    private void doDragFloatView(int movePos, View moveItem, boolean forceInvalidate) {
        ...
        if (updated) {
            ...
            // Log.d(TAG_DRAG_SORT, "adjust scroll = " + scroll);

            mLeListView.setSelectionFromTop(movePos, moveItem.getTop() + scroll - mLeListView.getPaddingTop());
            mLeListView.layoutChildren();
        }
    ...
    private void updateFloatView() {
        ...
        // restrict x motion
        int padLeft = mLeListView.getPaddingLeft();
        if (floatX > padLeft) {
            mFloatLoc.x = padLeft;
        } else if (floatX < padLeft) {
            mFloatLoc.x = padLeft;
        }
        ...
        final int numFooters = mLeListView.getFooterViewsCount();
        final int firstPos = mLeListView.getFirstVisiblePosition();
        final int lastPos = mLeListView.getLastVisiblePosition();

        // Log.d(TAG_DRAG_SORT, "nHead = " + numHeaders + " nFoot = " + numFooters + " first = " + firstPos + " last = " + lastPos);
        int topLimit = mLeListView.getPaddingTop();
        if (firstPos < numHeaders) {
            topLimit = mLeListView.getChildAt(numHeaders - firstPos - 1).getBottom();
        }
        if ((mDragFlags & DRAG_NEG_Y) == 0) {
            if (firstPos <= mSrcPos) {
                topLimit = Math.max(mLeListView.getChildAt(mSrcPos - firstPos).getTop(), topLimit);
            }
        }
        // bottom limit is top of first footer View or
        // bottom of last item in list
        int bottomLimit = mLeListView.getHeight() - mLeListView.getPaddingBottom();
        if (lastPos >= mLeListView.getCount() - numFooters - 1) {
            bottomLimit = mLeListView.getChildAt(mLeListView.getCount() - numFooters - 1 - firstPos).getBottom();
        }
        if ((mDragFlags & DRAG_POS_Y) == 0) {
            if (lastPos >= mSrcPos) {
                bottomLimit = Math.min(mLeListView.getChildAt(mSrcPos - firstPos).getBottom(), bottomLimit);
            }
        }

        // Log.d(TAG_DRAG_SORT, "dragView top = " + (y - mDragDeltaY));
        // Log.d(TAG_DRAG_SORT, "limit = " + limit);
        // Log.d(TAG_DRAG_SORT, "mDragDeltaY = " + mDragDeltaY);
    ...
    boolean isDragSortEnabled() {
        return mLeListView.getLeListViewMode() == LeListView.LE_DRAG_SORT;
    }
    ...
    private class DragScroller implements Runnable {
        ...
        public void stopScrolling(boolean now) {
            if (now) {
                mLeListView.removeCallbacks(this);
                mScrolling = false;
            } else {
                mAbort = true;
            }
        ...
        public void run() {
            ...
            if (scrollDir == UP) {
                View v = mLeListView.getChildAt(0);
                // Log.d(TAG_DRAG_SORT, "vtop = " + v.getTop() + " padtop = " + padTop);
                if (v == null) {
                    mScrolling = false;
                    return;
                } else {
                ...
                View v = mLeListView.getChildAt(last - first);
                if (v == null) {
                    mScrolling = false;
                    return;
                } else {
            ...
            final View moveItem = mLeListView.getChildAt(movePos - first);
            ...
            mLeListView.setSelectionFromTop(movePos, top - padTop);
            mLeListView.layoutChildren();
            mLeListView.invalidate();
            ...
            mPrevTime = mCurrTime;
            // Log.d(TAG_DRAG_SORT, "updated prevTime = " + mPrevTime);
     ...
     * {@link DragSortListView#setDragScrollProfile(com.letv.shared.widget.DragSortHelper.DragScrollProfile)} to
     * set custom profile.
     * 
     */
    ...
    public void removeCheckState(int position) {
        SparseBooleanArray cip = mLeListView.getCheckedItemPositions();
        ...
        for (int i = 0; i != runCount; i++) {
            if (!(runStart[i] == position || (runEnd[i] < runStart[i] && runEnd[i] > position))) {
                ...
                mLeListView.setItemChecked(rotate(runStart[i], -1, rangeStart, rangeEnd), true);
            }
            mLeListView.setItemChecked(rotate(runEnd[i], -1, rangeStart, rangeEnd), false);
        }
    }
    ...
    public void moveCheckState(int from, int to) {
        ...
        if (from < to) {
            ...
            for (int i = 0; i != runCount; i++) {
                mLeListView.setItemChecked(runStart[i], false);
                mLeListView.setItemChecked(runEnd[i], true);
            }
        }
    }
    ...
    boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (mFloatView != null) {
            ...
            if (mLeListView.onInterceptTouchEventSuper(ev)) {
    ...
    boolean onTouchEvent(MotionEvent ev) {
        ...
        if (!mDragEnabled) {
            return mLeListView.onTouchEventSuper(ev);
        }
        ...
        // LE_DRAG_SORT时，down在外面区域（如果menu打开，点击放menu的backView算是里面），关闭打开的menu
        /*if (action == MotionEvent.ACTION_DOWN && mSwipeFrontView > 0 && mSwipeBackView > 0) {// 有frontView和backView
            mDownPosition = pointToPosition(x, y);
            final int index = mDownPosition - getFirstVisiblePosition();
            final View child = getChildAt(index);
            if (child != null) {
                mFrontView = child.findViewById(mSwipeFrontView);
                
                if (closeOpenedItem(x)) {
                    return super.onTouchEvent(ev);// 如果关闭了，就结束此ontouch中的方法
                }
            }
        }*/
        ...
        if (mDragState == DRAGGING) {
            ...
            // if (mDragState != STOPPED) {
            if (mDragState == IDLE) {
                if (mLeListView.onTouchEventSuper(ev)) {
                    more = true;
                }
            }
