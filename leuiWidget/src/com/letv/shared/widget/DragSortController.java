...
/**
 * Class that starts and stops item drags on a {@link com.letv.shared.widget.LeListView}
 ...
 * {@link com.letv.shared.widget.LeListView#setTouchListener()} and
 * {@link com.letv.shared.widget.LeListView#setFloatViewManager()} of your
 * {@link com.letv.shared.widget.LeListView} instance.
 */
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
    ...
    private LeListView mDslv;
    private int mPositionX;

    /**
     * Calls {@link #DragSortController(com.letv.shared.widget.LeListView, int)} with a
     ...
     */
    public DragSortController(LeListView dslv) {
        this(dslv, 0, ON_DOWN);
    }

    public DragSortController(LeListView dslv, int dragHandleId, int dragInitMode) {
        this(dslv, dragHandleId, dragInitMode, 0);
    }
     ...
     */
    public DragSortController(LeListView dslv, int dragHandleId, int dragInitMode, int flingHandleId) {
        super(dslv);
        mDslv = dslv;
     ...
     * Starts the drag on the LeListView.
     *
    ...
    public boolean startDrag(int position, int deltaX, int deltaY) {
        ...
        int dragFlags = 0;
        if (mSortEnabled) {
            dragFlags |= DragSortHelper.DRAG_POS_Y | DragSortHelper.DRAG_NEG_Y;
        }

        mDragging = mDslv.getDragSortHelper().startDrag(position - mDslv.getHeaderViewsCount(), dragFlags, deltaX,
                deltaY);
        return mDragging;
    }
    ...
    public boolean onTouch(View v, MotionEvent ev) {
        if (!mDslv.getDragSortHelper().isDragSortEnabled() || mDslv.getDragSortHelper().isListViewIntercepted()) {
            return false;
        }
