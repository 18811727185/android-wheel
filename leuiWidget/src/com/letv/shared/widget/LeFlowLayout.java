...
// Designed by ApmeM in github https://github.com/ApmeM/android-flowlayout

public class LeFlowLayout extends ViewGroup {
    ...
    private int mMaxLineNum = INFINITE;

    public LeFlowLayout(Context context) {
        super(context);

        this.readStyleParameters(context, null);
    }

    public LeFlowLayout(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);

        this.readStyleParameters(context, attributeSet);
    }

    public LeFlowLayout(Context context, AttributeSet attributeSet, int defStyle) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (newLine) {
                lineNum ++;
                if (mMaxLineNum != INFINITE && lineNum > mMaxLineNum) {
                    for (int j=i; j<count; j++) {
                        View v = getChildAt(j);
                        if (v.getVisibility() == GONE) {
                            continue;
                        }
                        LayoutParams lp2 = (LayoutParams) v.getLayoutParams();
                        lp2.setExceedMaxLineNum(true);
                    }
                    break;
                }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            LayoutParams lp = (LayoutParams) child.getLayoutParams();
            if (lp.isExceedMaxLineNum()) continue;
    ...
    private void readStyleParameters(Context context, AttributeSet attributeSet) {
        TypedArray a = context.obtainStyledAttributes(attributeSet, R.styleable.LeFlowLayout);
        try {
            horizontalSpacing = a.getDimensionPixelSize(R.styleable.LeFlowLayout_leHorizontalSpacing, 0);
            verticalSpacing = a.getDimensionPixelSize(R.styleable.LeFlowLayout_leVerticalSpacing, 0);
            orientation = a.getInteger(R.styleable.LeFlowLayout_leOrientation, HORIZONTAL);
            debugDraw = a.getBoolean(R.styleable.LeFlowLayout_leDebugDraw, false);
            mMaxLineNum = a.getInteger(R.styleable.LeFlowLayout_leMaxLineNum, INFINITE);
        } finally {
            a.recycle();
        }
    }
    ...
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        private boolean exceedMaxLineNum;
        private int x;
        private int y;
        ...
        private void readStyleParameters(Context context, AttributeSet attributeSet) {
            TypedArray a = context.obtainStyledAttributes(attributeSet, R.styleable.LeFlowLayout_LayoutParams);
            try {
                horizontalSpacing = a.getDimensionPixelSize(R.styleable.LeFlowLayout_LayoutParams_layout_leHorizontalSpacing, NO_SPACING);
                verticalSpacing = a.getDimensionPixelSize(R.styleable.LeFlowLayout_LayoutParams_layout_leVerticalSpacing, NO_SPACING);
                newLine = a.getBoolean(R.styleable.LeFlowLayout_LayoutParams_layout_leNewLine, false);
            } finally {
                a.recycle();
            }
        }
        
        public void setExceedMaxLineNum(boolean exceed) {
            this.exceedMaxLineNum = exceed;
        }
        
        public boolean isExceedMaxLineNum() {
            return exceedMaxLineNum;
        }
    }
}
