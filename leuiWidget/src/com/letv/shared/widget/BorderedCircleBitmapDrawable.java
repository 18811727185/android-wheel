...
public class BorderedCircleBitmapDrawable extends Drawable {
    ...
    BorderedCircleBitmapDrawable(ColorDrawable colorDrawable, float circleRadius, int border,
            int borderColor,
    ...
    BorderedCircleBitmapDrawable(Bitmap bitmap, float circleRadius, int border, int borderColor,
            ScaleType scaleType) {
    ...
    public void draw(Canvas canvas) {
        ...
        if (mBorderWidth > 0) {
            canvas.drawCircle(
                    mBorderRect.left + mBorderRect.width() / 2,
                    mBorderRect.top + mBorderRect.height() / 2,
                    mCircleRadius,
                    mBorderPaint);
            canvas.drawCircle(
                    mDrawableRect.left + mDrawableRect.width() / 2,
                    mDrawableRect.top + mDrawableRect.height() / 2,
                    mCircleRadius - mBorderWidth,
                    mBitmapPaint);
        } else {
            canvas.drawCircle(
                    mDrawableRect.left + mDrawableRect.width() / 2,
        ...
        if (drawable != null) {
            if (drawable instanceof TransitionDrawable) {
                ...
                for (int i = 0; i < num; i++) {
                    ...
                    td.setId(i, i);
                    if (d instanceof ColorDrawable || d instanceof BorderedCircleBitmapDrawable) {
                        // skip colordrawables for now
                        drawableList[i] = d;
                    } else {
                        drawableList[i] = new BorderedCircleBitmapDrawable(
                                drawableToBitmap(td.getDrawable(i)), radius, border, borderColor,
                                scaleType);
                    }
            ...
            if (bm != null) {
                return new BorderedCircleBitmapDrawable(bm, radius, border, borderColor, scaleType);
            } else {
